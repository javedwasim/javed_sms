...
class Batches extends MY_Controller
{
    ...
    public function save_student_behavioural_score(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($result) {
                $json['success'] = true;
                $json['message'] = "Report save successfully!";
            } else {
                $json['error'] = true;
                $json['message'] = "Seems to an error.";
            }
    ...
    public function term_demographics(){
        $data = $this->input->post();
        //print_r($data);
        $id = $data['batch_id'];
        $term_id = $data['term_id'];
        ...
        $data['batch_id']=$id;
        $data['screen'] = 'batch_students';
        $json['result_html'] = $this->load->view('batches/student_grades', $data,true);
        if($this->input->is_ajax_request()) {
            set_content_type($json);
        }
    }
